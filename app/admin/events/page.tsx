...
export default function EventsPage() {
  ...
  const loadRsvps = async (eventId: number) => {
    const requestId = ++rsvpRequestRef.current;
    ...
    try {
      ...
      const nextRsvps = await rsvpRes.json() as Rsvp[];
      if (rsvpRequestRef.current === requestId) {
        setRsvps(nextRsvps);
      }
    } finally {
      if (rsvpRequestRef.current === requestId) {
